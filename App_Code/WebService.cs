...
public class WebService : System.Web.Services.WebService
{
    ...
    public string[] GetCompletionList(string prefixText)
    {
        ...
        cmd.Connection = con;
        con.Open();
        cmd.CommandText = "SELECT  C_Name,C_ContactNumber from CUSTOMER_DETAIL_MASTER Where C_Name like '" + prefixText + "%' or C_ContactNumber like '" + prefixText + "%'";
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            {
                st.Add(dt.Rows[i]["C_Name"].ToString());
                st.Add(dt.Rows[i]["C_ContactNumber"].ToString());
            }
        }
